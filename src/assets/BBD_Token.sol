...
contract BBDToken is ERC20 {
    ...
    address owner = 0x7E23E2781a1Da5DACe87b4f0e7bf68191263Fc32;
    address dev;
    ...
    modifier isOwner() {
        require(owner == msg.sender || dev == msg.sender, "Not allowed!");
        _;
    }

    constructor() ERC20("BBD Token", "BBD") {
        dev = msg.sender;
        _mint(dev, PUBLIC_SUPPLY); // 1 billion to owner
    ...
    modifier canTransfer(address sender, address recipient) {
        ...
        if (isDex[sender] && !buyingEnabled) {
            require(
                recipient == owner || managers[recipient],
                "Buying is disabled"
            );
        }
